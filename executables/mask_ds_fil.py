...
def myexecute(hotpotato, times_chunk, tstart, tstop, freqs_GHz, npol, nchans, chan_bw, n_bytes, hdr_size, nint_chunk, int_times_chunk, mask_zap_chans_per_intchunk, logger, rank):
    ...
    save_array = [data, freqs_GHz, times_chunk]
    save_keywords = ['DS', 'Freqs (GHz)', 'Time (s)']
...
def collate_npz(hdr, hotpotato, logger, remove_npz=True):
...
def divide_timechunks(nproc, nblocks, times, nint, int_times, mask_zap_chans_per_int, t_samp, downsample_time_factor):
    if nproc>1:
        equal_divide_size = np.round(len(times)/(nblocks*(nproc-1))).astype(int) # Divide the time axis into nblocks*(nproc -1) chunks.
    else:
        equal_divide_size =  np.round(len(times)/nblocks).astype(int) # Divide the time axis into nblocks chunks.
    # Increase chunk size, if required, to make it exactly divisible by the temporal downsampling factor.
    chunk_size = equal_divide_size + downsample_time_factor - (equal_divide_size % downsample_time_factor)
    ...
    while tstop <= len(times):
        tstart = tstop
        tstop = tstop+chunk_size
        tstart_values.append(tstart)
        tstop_values.append(tstop)
    # Drop samples from the final chunk to make its size divisible by the downsampling factor chosen.
    tstop_values[-1] = tstart+int(np.floor((len(times) - tstart)/downsample_time_factor)*downsample_time_factor)
...
def __MPI_MAIN__(parser):
    ...
    if rank==0:
        ...
        tstart_values, tstop_values, nint_values, int_times_values, mask_zap_chans_per_int_values = divide_timechunks(nproc, hotpotato['N_blocks'], times, nint, int_times, mask_zap_chans_per_int, t_samp, hotpotato['kernel_size_time_samples'])
        ...
        if nproc==1:
            for nb in range(len(tstart_values)):
                ...
                myexecute(hotpotato, times_chunk, tstart_values[nb], tstop_values[nb], freqs_GHz, npol, nchans, chan_bw, n_bytes, hdr_size, nint_values[nb], int_times_values[nb], mask_zap_chans_per_int_values[nb], parent_logger, rank)
        else:
            # Send data to child processors.
            for indx in range(1,nproc):
                comm.send((hotpotato, t_samp, tstart_values[indx-1], tstop_values[indx-1], freqs_GHz, npol, nchans, chan_bw, n_bytes, hdr_size, nint_values[indx-1], int_times_values[indx-1], mask_zap_chans_per_int_values[indx-1]), dest=indx, tag=indx)
            comm.Barrier() # Wait for all child processors to receive sent call.
            # Receive Data from child processors after execution.
            comm.Barrier()

        # Collate data from multiple .npz files output by child processors into a single filterbank file.
        collate_npz(hdr, hotpotato, parent_logger, remove_npz=True)
        ...
        hotpotato, t_samp, tstart_dist, tstop_dist, freqs_GHz, npol, nchans, chan_bw, n_bytes, hdr_size, nint_dist, int_times_dist, mask_zap_chans_per_int_dist = comm.recv(source=0, tag=rank)
        comm.Barrier()
        ...
        for nb in range(len(tstart_dist)):
            ...
            myexecute(hotpotato, times_chunk, tstart_dist[nb], tstop_dist[nb], freqs_GHz, npol, nchans, chan_bw, n_bytes, hdr_size, nint_dist[nb], int_times_dist[nb], mask_zap_chans_per_int_dist[nb], child_logger, rank)
